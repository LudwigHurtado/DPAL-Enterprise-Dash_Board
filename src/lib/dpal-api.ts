/**
 * DPAL Nexus API client — general health, probes, and reports feed.
 * Base URL resolution: localStorage override → NEXT_PUBLIC_DPAL_API_BASE env var.
 */
import { getApiBaseUrl } from './apiBase';

export { getApiBaseUrl as getApiBase };
...
export async function checkHealth(apiBase?: string): Promise<HealthResult> {
  ...
  if (!base) return { ok: false, status: 0, latencyMs: 0 };
...
export async function getReportsFeed(params: {
  ...
  const base = params.apiBase ?? getApiBaseUrl();
  if (!base) return { ok: false, items: [] };
  ...
  try {
    ...
    return { ok: false, items: [], error: e instanceof Error ? e.message : 'Network error' };
  }
}
...
export async function runProbes(apiBase?: string): Promise<ProbeResult[]> {
  ...
  const probe = async (name: string, fn: () => Promise<Response>): Promise<ProbeResult> => {
    const start = performance.now();
    try {
      const res = await fn();
      return { name, ok: res.ok, latencyMs: Math.round(performance.now() - start), status: res.status };
    } catch {
  ...
  const [health, reportsFeed] = await Promise.all([
    ...
    probe('reportsFeed', () => fetch(`${base}/api/reports/feed?limit=5`, { cache: 'no-store' })),
  ]);
